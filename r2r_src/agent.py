...
import model
import utils
from model import ObjEmbedding
from param import args
from utils import padding_idx, print_progress, get_glove_matrix
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20):
        ...
        self.objencoder = ObjEmbedding(glove_matrix).cuda()
    ...
    def _sort_batch(obs):
        ...
        return Variable(sorted_tensor, requires_grad=False).long().cuda(), mask.byte().cuda(), list(seq_lengths), list(
            perm_idx)
    ...
    def get_temp_obj_label(self, batch_size, max_cand, top_n_obj, obs):
    ...
    def get_cand_object_feat(self, obs):
        ...
        batch_size = len(obs)
        max_cand = max(candidate_leng)
        temp_obj_label = self.get_temp_obj_label(batch_size, max_cand, top_n_obj, obs)
    ...
    def make_equiv_action(self, a_t, perm_obs, perm_idx=None, traj=None):
        ...
        def take_action(k, id_x, name):
            if type(name) is int:  # Go to the next view
                self.env.env.sims[id_x].makeAction([name], [0], [0])
            else:  # Adjust
                self.env.env.sims[id_x].makeAction(*self.env_actions[name])
            state = self.env.env.sims[id_x].getState()[0]
            if traj is not None:
                traj[k]['path'].append((state.location.viewpointId, state.heading, state.elevation))
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        for t in range(self.episode_len):
            ...
            if self.feedback == 'teacher':
                ...
                a_t = a_t.detach()
                log_probs = torch_f.log_softmax(logit, 1)  # Calculate the log_prob here
                policy_log_probs.append(log_probs.gather(1, a_t.unsqueeze(1)))  # Gather the log_prob for each batch
            elif self.feedback == 'sample':
                probs = torch_f.softmax(logit, 1)  # sampling an action from model
    ...
    def zero_grad(self):
        self.loss = 0.
        self.losses = []
        for m, optimizer in zip(self.models, self.optimizers):
            m.train()
            optimizer.zero_grad()
    ...
    def train(self, n_iters, feedback='teacher', **kwargs):
        ...
        for iter_num in range(1, n_iters + 1):
            ...
            print_progress(iter_num, n_iters + 1, prefix='Progress:', suffix='Complete', bar_length=50)
    ...
    def save(self, epoch, path):
        ...
        def create_state(name, model_loc, optimizer):
            states[name] = {
                'epoch': epoch + 1,
                'state_dict': model_loc.state_dict(),
    ...
    def load(self, path):
        ...
        def recover_state(name, model_loc, optimizer):
            state = model_loc.state_dict()
            model_keys = set(state.keys())
            load_keys = set(states[name]['state_dict'].keys())
            if model_keys != load_keys:
                print("NOTICE: DIFFERENT KEYS IN THE LISTEREN")
            state.update(states[name]['state_dict'])
            model_loc.load_state_dict(state)
            if args.loadOptim:
